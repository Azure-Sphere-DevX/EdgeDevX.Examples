...
void update_lcd_panel(int counter)
{
    ...
    environment.latest.temperature = (int)avnet_get_temperature();
    environment.latest.pressure = (int)avnet_get_pressure();

    co2_read(&environment);

    log_write((uint8_t *)&environment.latest, sizeof(ENVIRONMENT_T), counter);

    // clear environment struct to demostrate reading back from mutable storage
    memset(&environment, 0x00, sizeof(ENVIRONMENT_T));

    log_read((uint8_t *)&environment.latest, sizeof(ENVIRONMENT_T), counter);
    

    if (last_temperature == environment.latest.temperature && last_CO2_ppm == environment.latest.co2ppm)
    {
        return;
    }

    last_temperature = environment.latest.temperature;
    last_pressure = environment.latest.pressure;
